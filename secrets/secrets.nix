# SSH public keys for agenix encryption
# These keys determine who can decrypt each secret

let
  # User keys (for personal access)
  kelvin_nixia = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINlD7buD+WXmzv0HW6Ns/LKPbHfqh7Va8JIxNzTY1zsV kelvin@nixia";
  kelvin_mako = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDYf1GTjqmCH89LG0v1zMWL6detrFjYCaL9+A5qmIqNM kelvin@mako-wsl";
  kelvin = [ kelvin_nixia kelvin_mako ];
  users = kelvin;

  # Machine host keys (RSA)
  kazuma = "ssh-rsa ";
  nixia = "ssh-rsa ";
  systems = [ kazuma nixia ];

  # Server host keys
  stratus = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIDN73vUDMvgigSgxXsTzM805QLMizJupFnpwFiCelvuf";
  servers = [ stratus ];
in
{
  "duckdns-token-kelvin.age".publicKeys = kelvin ++ systems;
  "dynv6-token-kelvin.age".publicKeys = kelvin ++ systems ++ servers;
}
